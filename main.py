...
async def hsoat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id, usuario = obtener_usuario(update, usuarios)
    if len(context.args) != 1:
        return await responder_error(update, "Uso: /hsoat PLACA")
    ...
    try:
        ...
        if not data.get("success"):
            return await editar_error(mensaje, "Placa no encontrada en SOAT.")
...
async def suel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not data.get("success"):
            return await editar_error(mensaje, "Sin registros laborales.")
...
async def denpla(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not data.get("success"):
            return await editar_error(mensaje, "Sin denuncias para esta placa.")
...
async def facial(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not data.get("success"):
            return await editar_error(estado, "No se encontraron coincidencias faciales.")
...
async def revtec(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id, usuario = obtener_usuario(update, usuarios)
    if len(context.args) != 1:
        return await responder_error(update, "Uso: /revtec PLACA")
    placa = context.args[0].strip().upper()
    ...
    try:
        ...
        if not data.get("success"):
            return await editar_error(mensaje, "Sin historial de revisiones.")
