...
@app.route('/')
def home():
    return "🔥 SPECTER PERÚ BOT ACTIVO 24/7 - CODART X V1 FUTURISTA"
...
#============================================================
#ESTILO FUTURISTA CENTRALIZADO - BOTONES VOLVER A MENU
#============================================================
SEPARADOR = "━━━━━━━━━━━━━━━━━━━━━━"
SEPARADOR_CORTO = "━━━━━━━━━━━━━━━━━━"
...
BTN_VOLVER_CMDS = InlineKeyboardMarkup(
    [
        [InlineKeyboardButton("🏠 MENÚ PRINCIPAL", callback_data="volver_cmds")],
        [InlineKeyboardButton("💎 COMPRAR CRÉDITOS", callback_data="cmd_buy")]
    ]
)
...
async def consultar_api_get(url: str, timeout: float = 35.0) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"error": "📡 No se pudo conectar a api-codart.cgrt.org."}
...
#============================================================
#UTILIDADES DE RESPUESTA CON BOTÓN VOLVER
#============================================================
...
async def editar_error(mensaje_obj, mensaje_error: str) -> None:
    try:
        await mensaje_obj.edit_text(
...
#============================================================
#COMANDOS DE CONSULTA IMPLEMENTADOS - TODAS LAS APIS REALES CON BOTÓN VOLVER
#============================================================
...
async def saldo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"{titulo_sistema('TU SALDO', '💰')}\n\n"
        f"💰 <b>Créditos:</b> <code>{saldo_actual}</code>\n"
        f"📱 <b>Número:</b> <code>{celular}</code>\n"
        f"📊 <b>Consultas:</b> <code>{usuario.get('consultas',0)}</code>\n\n"
    ...
    try:
        ...
        if images:
            try:
                ...
                await update.message.reply_photo(photo=BytesIO(raw), caption="📸 Foto RENIEC", reply_markup=BTN_VOLVER)
    ...
    try:
        ...
        for img in info.get("images", [])[:4]:
            try:
                ...
                await update.message.reply_photo(photo=BytesIO(raw), reply_markup=BTN_VOLVER)
            except Exception:
                continue
    except Exception as e:
...
async def telpcel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 1:
        return await update.message.reply_text(
            f"{titulo_sistema('TELPCEL', '📱')}\n\nUso: <code>/telpcel 900000001</code>\n💎 Costo: {PRECIOS['telpcel']} crd",
    ...
    try:
        ...
        if res.get("images"):
            try:
                ...
                await update.message.reply_photo(photo=BytesIO(raw), caption="📸 Foto AGV", reply_markup=BTN_VOLVER)
            except Exception:
                pass
    except Exception as e:
    ...
    try:
        ...
        if res.get("images"):
            for img in res["images"]:
                try:
                    raw = extraer_data_uri(img.get("data_uri"))
                    await update.message.reply_photo(photo=BytesIO(raw), caption="📸 DNIVEL", reply_markup=BTN_VOLVER)
                except Exception:
                    continue
    except Exception as e:
...
async def rqh(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        texto = (
            ...
            f"📊 <b>RESUMEN:</b> Total:{resumen.get('total',0)} Activas:{resumen.get('activas',0)}\n"
            f"{SEPARADOR}\n"
        )
        for d in detalles:
            texto += (
                ...
                f"🔢 <b>EXP:</b> <code>{error_html(d.get('exp'))}</code>\n"
...
#============================================================
#COMANDOS GENERALES FUTURISTAS CON BOTÓN VOLVER
#============================================================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    usuarios = cargar_usuarios()
    obtener_usuario(update, usuarios)
    texto = (
        f"{titulo_sistema('SPECTER PERÚ', '⚜️')}\n\n"
        "🚀 <b>PLATAFORMA DE CONSULTAS PERÚ</b>\n"
        "🛰️ <b>CODART X V1 • FUTURISTA EDITION</b>\n\n"
        f"🏷️ Bot: <b>{html.escape(BOT_NAME)}</b>\n"
        f"👤 User: <b>{html.escape(BOT_USER)}</b>\n"
        "📡 Estado: <b>🟢 ONLINE 24/7</b>\n"
        "⚡ Latencia: <b>ULTRA BAJA</b>\n\n"
        f"{SEPARADOR}\n"
        ...
        "👑 /staff ➜ Staff oficial\n\n"
        f"{SEPARADOR}\n"
        "🔥 <i>Sistema potenciado por CODART API</i>"
    )
...
async def cmds(update: Update, context: ContextTypes.DEFAULT_TYPE):
    texto = (
        ...
        "👇 Toca un botón para ver comandos y precios\n\n"
        f"{SEPARADOR}\n"
        "🪪 RENIEC → DNI, DNIT, AGV, DNIVEL\n"
        "🚘 VEHÍCULOS → SOAT, DENPLA, REVTEC\n"
        "📱 TELÉFONO → TELP, TELPCEL\n"
        "⚖️ DENUNCIAS → DEN, DENUNCIAS\n"
        "💰 LABORAL → SUEL\n"
        "🧬 BIOMÉTRICO → FACIAL\n"
        "🔍 OTROS → DIR, RQH"
    )
...
async def me(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    texto = (
        ...
        f"👤 <b>Nombre:</b> <code>{error_html(usuario.get('nombre', 'Usuario'))}</code>\n"
        f"🆔 <b>ID:</b> <code>{user_id}</code>\n"
        f"🔗 <b>Username:</b> <code>{error_html(usuario.get('username','-'))}</code>\n"
        f"📱 <b>Celular:</b> <code>{error_html(usuario.get('celular','No registrado'))}</code>\n"
        f"💳 <b>Créditos:</b> <code>{usuario.get('creditos', 0)}</code>\n"
        f"📊 <b>Consultas:</b> <code>{usuario.get('consultas', 0)}</code>\n"
        f"⭐ <b>Plan:</b> <code>{error_html(usuario.get('plan', 'FREE'))}</code>\n"
        f"📅 <b>Registro:</b> <code>{usuario.get('fecha_registro','-')[:10]}</code>\n"
        f"{SEPARADOR}\n"
        "⚜️ <b>SPECTER PERÚ • CODART X V1</b>"
    )
...
async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    texto = (
        ...
        "💰 <b>TABLA DE CRÉDITOS</b>\n"
        f"{SEPARADOR}\n"
        "🥉 <b>BÁSICO</b> → 100 crd ➜ <code>S/ 10</code>\n"
        "🥈 <b>PRO</b> → 200 crd ➜ <code>S/ 20</code>\n"
        "🥇 <b>ELITE</b> → 400 crd ➜ <code>S/ 30</code>\n"
        "💎 <b>DIAMANTE</b> → 1000 crd ➜ <code>S/ 60</code>\n\n"
        f"{SEPARADOR}\n"
        "💳 <b>MÉTODOS DE PAGO</b>\n"
        "📱 Yape • Plin • Transferencia\n"
        f"📞 Número: <code>{TU_CELULAR_YAPE}</code>\n"
        f"👤 Titular: <b>{TU_NOMBRE}</b>\n\n"
        "⚡ <b>RECARGA AUTOMÁTICA</b>\n"
        "1️⃣ Guarda tu número: <code>/micelular 9XXXXXXXX</code>\n"
        "2️⃣ Paga y escribe: <code>/pagar 10</code>\n"
        "3️⃣ Créditos se suman SOLOS\n\n"
        "👤 <b>ADMIN:</b> @Sthep_18"
    )
    await update.message.reply_text(texto, parse_mode="HTML", reply_markup=BTN_VOLVER)
...
async def staff(update: Update, context: ContextTypes.DEFAULT_TYPE):
    texto = (
        ...
        "👑 <b>@Sthep_18</b> - Fundador\n"
        "⚡ Dev Senior • Backend • APIs\n\n"
        "👨‍💻 <b>DESARROLLO</b>\n"
        f"🤖 Bot: <code>{BOT_USER}</code>\n"
        "🛰️ API: CODART X V1\n"
        "🔥 Versión: FUTURISTA 2026\n\n"
        "📞 <b>SOPORTE 24/7</b>\n"
        "💬 Telegram: @Sthep_18\n"
        "📧 Consultas, bugs, ventas\n\n"
        f"{SEPARADOR}\n"
        "⚜️ <b>SPECTER PERÚ - ELITE TEAM</b>"
    )
    await update.message.reply_text(texto, parse_mode="HTML", reply_markup=BTN_VOLVER)
...
async def addcreditos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args or []
    if len(args) != 2:
        await update.message.reply_text("Uso: /addcreditos <user_id> <cantidad>", reply_markup=BTN_VOLVER)
        return
    target_id = args[0]
    try:
        cantidad = int(args[1])
    except ValueError:
        await update.message.reply_text("❌ La cantidad debe ser un número entero.", reply_markup=BTN_VOLVER)
        return
    usuarios = cargar_usuarios() or {}
    if target_id not in usuarios:
        await update.message.reply_text(f"❌ No existe el usuario {target_id}.", reply_markup=BTN_VOLVER)
        return
    usuarios[target_id]["creditos"] = int(usuarios[target_id].get("creditos", 0)) + cantidad
    guardar_usuarios(usuarios)
    await update.message.reply_text(f"✅ Agregados {cantidad} a {target_id}", reply_markup=BTN_VOLVER)
...
async def quitarcrd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args or []
    if len(args) != 2:
        await update.message.reply_text("Uso: /quitarcrd <user_id> <cantidad>", reply_markup=BTN_VOLVER)
        return
    target_id = args[0]
    try:
        cantidad = int(args[1])
    ...
    guardar_usuarios(usuarios)
    await update.message.reply_text(f"✅ Quitados {cantidad} a {target_id}", reply_markup=BTN_VOLVER)

#============================================================
#MENÚ INTERACTIVO CALLBACKS - TODOS CON BOTÓN VOLVER
#============================================================
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "cmd_me":
        ...
        texto = (
            ...
            f"💳 <b>Créditos:</b> <code>{usuario.get('creditos',0)}</code>\n"
            f"📊 <b>Consultas:</b> <code>{usuario.get('consultas',0)}</code>\n"
            f"📱 <b>Celular:</b> <code>{error_html(usuario.get('celular','-'))}</code>\n"
        )
        return await query.edit_message_text(texto, parse_mode="HTML", reply_markup=BTN_VOLVER)

    textos = {
        "cmd_reniec": (
            f"{titulo_sistema('RENIEC', '🪪')}\n\n"
            f"🪪 /dni ➜ {PRECIOS['dni']} crd - DNI básico + foto\n"
            f"💳 /dnit ➜ {PRECIOS['dnit']} crd - DNI detallado 4 fotos\n"
            f"🛰️ /agv ➜ {PRECIOS['agv']} crd - Consulta AGV\n"
            f"📊 /dnivel ➜ {PRECIOS['dnivel']} crd - DNI Nivel\n\n"
            f"{SEPARADOR}\n"
        ...
        "cmd_vehiculos": (
            ...
            f"🚗 /hsoat ➜ {PRECIOS['hsoat']} crd - Historial SOAT\n"
            f"🚨 /denpla ➜ {PRECIOS['denpla']} crd - Denuncias por placa\n"
            f"🛠️ /revtec ➜ {PRECIOS['revtec']} crd - Revisión técnica\n\n"
            f"{SEPARADOR}\n"
            "Ejemplo: <code>/hsoat D5G960</code>"
        ),
        "cmd_telefono": (
            f"{titulo_sistema('TELEFONÍA', '📱')}\n\n"
            f"📡 /telp ➜ {PRECIOS['telp']} crd - Líneas por DNI\n"
            f"📱 /telpcel ➜ {PRECIOS['telpcel']} crd - Titular por celular\n\n"
            f"{SEPARADOR}\n"
            "Ejemplo: <code>/telpcel 900000001</code>"
        ),
        "cmd_denuncia": (
            f"{titulo_sistema('DENUNCIAS', '⚖️')}\n\n"
            f"🚨 /den ➜ {PRECIOS['denuncia']} crd - Historial texto\n"
            f"📂 /denuncias ➜ {PRECIOS['denuncias']} crd - PDFs oficiales\n\n"
            f"{SEPARADOR}\n"
            "Ejemplo: <code>/den 12345678</code>"
        ),
        "cmd_sueldo": (
            f"{titulo_sistema('SUELDOS', '💰')}\n\n"
            f"💼 /suel ➜ {PRECIOS['suel']} crd - Reporte laboral SUNAT\n\n"
            f"{SEPARADOR}\n"
            "Ejemplo: <code>/suel 12345678</code>"
        ),
        "cmd_facial": (
            f"{titulo_sistema('FACIAL', '🧬')}\n\n"
            f"🧬 /facial ➜ {PRECIOS['facial']} crd - Reconocimiento facial\n\n"
            "📷 Envía una foto con caption <code>/facial</code>\n"
            "🎯 Te devuelve coincidencias con %"
        ),
        "cmd_otros": (
            f"{titulo_sistema('OTROS', '🔍')}\n\n"
            f"📍 /dir ➜ {PRECIOS['dir']} crd - Direcciones históricas\n"
            f"👮 /rqh ➜ {PRECIOS['rqh']} crd - Requisitorias\n\n"
            f"{SEPARADOR}\n"
            "Ejemplo: <code>/dir 12345678</code>"
        ),
        "cmd_buy": (
            f"{titulo_sistema('PLANES PREMIUM', '💎')}\n\n"
            "🥉 100 crd → S/10\n"
            "🥈 200 crd → S/20\n"
            "🥇 400 crd → S/30\n\n"
            "Usa /buy para más info\n"
            "Guarda tu cel: /micelular 9XXXXXXXX"
        )
    }
...
def main():
    if not BOT_TOKEN:
        raise RuntimeError("Falta BOT_TOKEN - Configura variable de entorno BOT_TOKEN")
    keep_alive()
    application = ApplicationBuilder().token(BOT_TOKEN).build()
    # Comandos generales futuristas
    ...
    # Consultas - TODAS LAS APIS REALES CODART X V1
    ...
    logger.info("🚀 SPECTER PERÚ FUTURISTA ONLINE - CODART X V1 COMPLETO")
